...
const labelNames = ['client_name', 'baseUrl', 'url', 'method', 'status_code', 'status_message', 'error_name']
const buckets = [0.1, 0.25, 0.5, 1, 2, 5, 10]

const apiMetrics = new prometheus.Histogram({
  name: 'api_requests',
...
const requestMetrics = new prometheus.Histogram({
...
class FBJWTClient {
   ...
   *
   **/
  createRequestOptions (urlPattern, urlKeys, data = {}, searchParams) {
   ...
   *
   **/
  send (method, urlPattern, urlKeys, payload, sendOptions) {
    // response.retryCount
    // response.timings
    const client = this
    const options = this.createRequestOptions(urlPattern, urlKeys, payload, method === 'get')

    let requestMetricsEnd

    const labels = {
      client_name: this.constructor.name,
   ...
   *
   **/
  sendGet (urlPattern, urlKeys, payload, sendOptions) {
    return this.send('get', urlPattern, urlKeys, payload, sendOptions)
  }
   ...
   *
   **/
  sendPost (urlPattern, urlKeys, payload, sendOptions) {
    return this.send('post', urlPattern, urlKeys, payload, sendOptions)
  }
