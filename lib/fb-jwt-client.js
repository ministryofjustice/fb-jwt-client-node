...
class FBJWTClient {
   ...
   *
   **/
  send (method, urlPattern, urlKeys, payload) {
    // TODO: metrics for this.constructor.name
    // response.retryCount
    // response.timings
    const client = this
    const options = this.createRequestOptions(urlPattern, urlKeys, payload, method === 'get')
    return got[method](options)
  ...
  sendGet (urlPattern, urlKeys, payload) {
    return this.send('get', urlPattern, urlKeys, payload)
  }
  ...
  sendPost (urlPattern, urlKeys, payload) {
    return this.send('post', urlPattern, urlKeys, payload)
  }
