  ...
  send (method, args, logger) {
    ...
    return got[method](gotOptions)
      ...
      .catch(e => {
        ...
        if (logger) {
          const errorObj = typeof e.error === 'object' ? JSON.stringify(e.error) : ''
          const logObject = Object.assign({}, labels, {err: e})
          logger.error(logObject, `Error: ${this.constructor.name}: ${e.name} - ${e.statusCode} - ${e.statusMessage} - ${errorObj}`)
        }
