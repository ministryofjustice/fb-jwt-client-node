...
class FBJWTClient {
  ...
  logError (type, e, labels, logger) {
    const errorResponse = e.error || e.body
    const errorObj = typeof errorResponse === 'object' ? JSON.stringify(errorResponse) : ''
    const logObject = Object.assign({}, labels, {err: e})
    const excludes = ['gotOptions', 'url', 'path']
    excludes.forEach(exclude => {
      delete logObject.err[exclude]
    })
    logger.error(logObject, `JWT ${type} request error: ${this.constructor.name}: ${labels.method.toUpperCase()} ${labels.baseUrl}${labels.url} - ${e.name} - ${e.code ? e.code : ''} - ${e.statusCode ? e.statusCode : ''} - ${e.statusMessage ? e.statusMessage : ''} - ${errorObj}`)
  }
