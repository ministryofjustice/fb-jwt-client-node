...
const getResponseLabels = (response) => {
  ...
  if (
    ...
    if (response.statusCode) responseLabels.status_code = Reflect.get(response, 'statusCode')
    if (response.statusMessage) responseLabels.status_message = Reflect.get(response, 'statusMessage')
  } else {
    if (response instanceof Error) {
      if (Reflect.has(response, 'body')) {
        ...
        responseLabels.error_code = getErrorStatusCode(Reflect.get(response, 'code'))
        responseLabels.error_message = Reflect.get(response, 'message')
      }
    }
  }
