...
const serviceUrl = 'https://microservice'
const createEndpointUrl = `${serviceUrl}/service/${serviceSlug}/user/${userId}`
const data = {foo: 'bar'}
...
test('When instantiating client without a service url', t => {
  testInstantiation(t, [serviceToken], 'ENOSERVICEURL', 'No service url passed to client')
})

test('When instantiating client without a service slug', t => {
  testInstantiation(t, [serviceToken, serviceUrl], 'ENOSERVICESLUG', 'No service slug passed to client')
})
...
test('When instantiating client with a custom error', t => {
  ...
  try {
    const jwtClient = new FBJWTClient(null, serviceUrl, serviceSlug, MyError) // eslint-disable-line no-unused-vars
  } catch (e) {
...
const jwtClient = new FBJWTClient(serviceToken, serviceUrl, serviceSlug)
...
test('When sending gets', async t => {
  ...
  t.equal(callArgs.url, `${serviceUrl}/user/testUserId`, 'it should call the correct url')
